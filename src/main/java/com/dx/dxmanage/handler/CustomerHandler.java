	...
	public int addCustomer(Customer customer, String newcode) {
		...
		try {
			...
			String customername = Encode.BASE64ToUtf8(customer.getCustomername().replaceAll(" ", "+"));
			 ...
			 if(customer.getCustomername1()!=null) {
				 customername1 = customername;// 设置初始单位开票名称
			 }
			...
			if (customer2.isEmpty()) {// 当系统中不存在此单位时
				if ("".equals(newcode) || newcode == null) {
					...
					List<Customer> customer3 = customerService.selectCusByMemberAndCode(membership, newcode);
			...
			if(a==1) {
				logger.info("人社局添加/修改单位：membership="+membership+"&customercode="+customercode+"&customername="+customername+"&newcode="+newcode);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
